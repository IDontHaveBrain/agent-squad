...
get_latest_version() {
    ...
    API_RESPONSE=$(curl -sS "https://api.github.com/repos/IDontHaveBrain/agent-squad/releases")
    if [ $? -ne 0 ]; then
...
main() {
    ...
    RELEASE_URL="https://github.com/IDontHaveBrain/agent-squad/releases/download/v${VERSION}"
    ARCHIVE_NAME="agent-squad_${VERSION}_${PLATFORM}_${ARCHITECTURE}${ARCHIVE_EXT}"
